...
const gameBoard = (() => {
    ...
    const check_for_winner = () => {
        ...
        if ((_ifVertical(length, gameController.player_marker)) || (_ifHorizontal(length,gameController.player_marker)) || (_ifDiagonal(length, gameController.player_marker))) { 
        ...
        } else if ((_ifVertical(length, gameController.computer_marker)) || (_ifHorizontal(length,gameController.computer_marker)) || (_ifDiagonal(length, gameController.computer_marker))) {  
            display_winner.style.display = 'block';
...
const displayController = (() => {
    ...
    const _create_cell = (cell, row, column) => {
        ...
        cell.addEventListener('click', () => {
            if (gameBoard.get_won_already()) {
                ...
                gameController.game_is_played(gameController.player_marker,row, column);
                if (!gameBoard.check_if_filled() && gameBoard.get_won_already() == false) {
    ...
    const draw_scores = (player_score,tie_score,computer_score) => {
        console.log("drawing these scores",player_score,tie_score,computer_score );
    ...
    return {
        draw_board, start_new_game, draw_scores,
    };
...
const gameController = (() => {
    ...
    const loadfromStorage = () => {
        ...
        computerScoreFromStorage = JSON.parse(localStorage.getItem("computerScore"));
        tieScoreFromStorage = JSON.parse(localStorage.getItem("tieScore"));
        if ((playerScoreFromStorage !== null) || (computerScoreFromStorage != null) || (tieScoreFromStorage != null)){
            player_score = playerScoreFromStorage;
            computer_score = computerScoreFromStorage;
            tie_score = tieScoreFromStorage;
        } else if ((playerScoreFromStorage == null) && (computerScoreFromStorage == null) &&  (tieScoreFromStorage == null)) {
            player_score = 0;
            computer_score = 0;
            tie_score = 0;
        }
        displayController.draw_scores(playerScoreFromStorage,tieScoreFromStorage,computerScoreFromStorage);
    }
...
const computerController = (() => {
    const insert_marker = (marker) => {
        ...
        let random_selection = random_arr[_get_random_coordinates(random_arr.length)];
